"""Nasdaq Data Link Sharadar loaders.

Independent data backend for the SFA bundle.

Goals:
- Keep the existing yfinance flow untouched.
- Load Sharadar SEP prices in the same MultiIndex shape as data.load_prices().
- Load Sharadar SF1 fundamentals as *history* and build point-in-time snapshots
  using ``datekey`` so backtests do not see future data.

Environment:
    NASDAQ_DATA_LINK_API_KEY or QUANDL_API_KEY
"""
...
_FUND_CACHE_TTL = 86400  # 24 hours
...
def _fetch_table(table: str, params: dict[str, str], paginate: bool = True) -> pd.DataFrame:
    """Fetch a Nasdaq Data Link datatable into a DataFrame."""
...
def load_prices(tickers: list[str], start: str, end: str) -> pd.DataFrame:
    """Sharadar SEP daily prices in (ticker, field) format."""
...
def load_tickers_metadata(tickers: list[str]) -> pd.DataFrame:
    """Ticker metadata used to enrich PIT fundamentals with sector."""
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    params = {"ticker": ",".join(sorted(tickers))}
...
def load_fundamentals_history(
    ...
    if df.empty:
        ...
        meta = load_tickers_metadata(tickers)
        if not meta.empty:
...
def select_fundamentals_snapshot(
    ...
    sort_cols = [c for c in ["ticker", "datekey", "calendardate", "_dimension_priority"] if c in hist.columns]
    ascending_map = {
        "ticker": True,
        "datekey": False,
        "calendardate": False,
    ...
    market_cap = _first_present(latest, ["marketcap", "market_cap"])
    fcf = _first_present(latest, ["fcf", "freecashflow", "free_cash_flow"])
...
def load_fundamentals(tickers: list[str], as_of_date: str | pd.Timestamp | None = None) -> pd.DataFrame:
    """Convenience wrapper returning a snapshot, not full history."""
