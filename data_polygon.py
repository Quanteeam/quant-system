"""Polygon.io 데이터 로딩 (Phase 6).

yfinance 대비 장점:
- Point-in-time (PIT) fundamental → look-ahead bias 제거
- Survivorship-bias-free universe
- 더 긴 earnings 이력

환경 변수:
    POLYGON_API_KEY — Polygon Stocks Starter 이상 필요 ($170+/월)

동일 인터페이스: load_prices(), load_fundamentals(), load_earnings()
→ data.py 대신 이 모듈을 import하면 됨.
"""
...
def _get_client():
    ...
    if not key:
        raise EnvironmentError("POLYGON_API_KEY 환경 변수 필요")
...
def load_fundamentals(tickers: list[str]) -> pd.DataFrame:
    ...
    Point-in-time: filedate 기준으로 해당 시점에 공개된 데이터만 사용.
    → look-ahead bias 제거.
    """
...
def load_earnings(tickers: list[str]) -> pd.DataFrame:
    ...
    Sharadar SEP 테이블 사용: 과거 전체 분기 이력 제공.
    """
    h = hashlib.md5(json.dumps(sorted(tickers)).encode()).hexdigest()
