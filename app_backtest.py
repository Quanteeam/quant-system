...
def render_backtest(
    ...
    info_cols[0].metric("Eval Window", f"{pd.Timestamp(start).date()} -> {pd.Timestamp(end).date()}")
    info_cols[1].metric("Load Start", str(pd.Timestamp(run_info.get("load_start", start)).date()))
    info_cols[2].metric("Universe Mode", "Dynamic" if run_info.get("dynamic_universe") else "Legacy")
    info_cols[3].metric("Candidates", f"{run_info.get('candidate_count', 0):,}")
    ...
    benchmark_colors = {"SPY": "#64748b", "QQQ": "#c026d3", "IWM": "#0ea5e9"}
