...
def _get_close(prices: pd.DataFrame) -> pd.DataFrame:
    """prices MultiIndex?먯꽌 close 異붿텧 (adj_close ?곗꽑)."""
    for field in ("adj_close", "close"):
        try:
            return prices.xs(field, level="field", axis=1)
        except KeyError:
            continue
    raise KeyError("prices??close ?먮뒗 adj_close 而щ읆???놁뒿?덈떎.")


def _zscore(s: pd.Series) -> pd.Series:
    """Cross-sectional z-score. NaN? ?쒖쇅, ?쒖??몄감 0?대㈃ 0 諛섑솚."""
    valid = s.dropna()
...
def _sector_neutral_zscore(raw: pd.Series, sectors: pd.Series) -> pd.Series:
    """Sector ??z-score (踰≫꽣??. sector ?뺣낫 ?녿뒗 醫낅ぉ? ?꾩껜 z-score ?곸슜."""
...
def compute_momentum(
    ...
    """12-1 momentum: lookback???섏씡, 吏곸쟾 skip???쒖쇅."""
...
def compute_size(fundamentals: pd.DataFrame) -> pd.Series:
    """Size factor: -log(market_cap). ?뚰삎二쇱씪?섎줉 ?믪? ?먯닔."""
...
def compute_lowvol(prices: pd.DataFrame, lookback: int = 60) -> pd.Series:
    """Low volatility: -annualized_vol(lookback days). ??? 蹂?숈꽦 ?좏샇."""
    close = _get_close(prices)
    if len(close) < lookback + 1:
        return pd.Series(dtype=float)
...
def compute_composite(
    ...
    """5-factor weighted composite score. ?믪쓣?섎줉 留ㅼ닔 ?꾨낫."""
    if weights is None:
